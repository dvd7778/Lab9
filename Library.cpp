...
int Library::binarySearchHelper(vector<Book> bookshelf, Book target, int left, int right) {
    ...
    if(target.compareTo(bookshelf[half]) == 0){
        return half;
    }
    ...
    return binarySearchHelper(bookshelf, target, left, half);    
}   

/*if(bookshelf.size()<=2 && bookshelf[0] != target && bookshelf[1] != target){
        return -1;
    }
    if(target.compareTo(bookshelf[left-1]) == 0){
        return left-1;
    }
    if(target.compareTo(bookshelf[right-1]) == 0){
        return right-1;
    }

    if(target.compareTo(bookshelf[left-1]) > 0){
        bookshelf.erase(bookshelf.begin(), bookshelf.begin() + left -1);
        return left-1 + binarySearchHelper(bookshelf, target, bookshelf.size()/2, bookshelf.size()/2 +1);
    }

    if(target.compareTo(bookshelf[right-1]) > 0){
        bookshelf.erase(bookshelf.begin(), bookshelf.begin() + left -1);
        return binarySearchHelper(bookshelf, target, bookshelf.size()/2, bookshelf.size()/2 +1) + ;
    } */
...
vector<Book> Library::bubbleSort(vector<Book> bookshelf) {
    ...
    for(int i=0; i<bookshelf.size(); i++){
        for(int j=1; j<bookshelf.size(); j++){
            if(bookshelf[j-1].compareTo(bookshelf[j]) > 0){
                swap = bookshelf[j-1];
